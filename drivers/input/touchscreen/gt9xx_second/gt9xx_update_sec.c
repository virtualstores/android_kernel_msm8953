 ...
 *      V1.6:
 *          1. replace guitar_client with gtp_sec_i2c_connect_client;
...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
	...
	if (i == GOODIX_MAX_CFG_GROUP) {
		...
		ret = gtp_sec_i2c_read_dbl_check(client, GTP_REG_SENSOR_ID,
							&sensor_id, 1);
		if (SUCCESS == ret) {
	...
	ret = gtp_sec_i2c_read_dbl_check(client, GTP_REG_CONFIG_DATA,
					&opr_buf[0], 1);
	if (ret == SUCCESS) {
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
	...
	ret = gtp_sec_i2c_read_dbl_check(client, GUP_REG_HW_INFO,
	...
	/*  step3:get pid & vid */
	ret = gtp_sec_i2c_read_dbl_check(client, GUP_REG_PID_VID,
					...
					const struct firmware *cfg)
{
	...
	pr_debug("config file ASCII len:%lu", cfg->size);
		...
		struct st_update_msg *msg, u8 *path)
{
	...
	dev_dbg(&client->dev, "Config File: %s size=%lu", path, fw->size);
	msg->fw_data =
...
s32 gup_update_proc(void *dir)
{
	...
	ret = gup_check_update_file(gtp_sec_i2c_connect_client, &fw_head, (u8 *)dir);
	if (ret == FAIL) {
		pr_err("check update file fail.");
		goto file_fail;
	}

	/* gtp_sec_reset_guitar(gtp_sec_i2c_connect_client, 20); */
	ret = gup_get_ic_fw_msg(gtp_sec_i2c_connect_client);
	if (ret == FAIL) {
...
#endif
	ret = gup_enter_update_mode(gtp_sec_i2c_connect_client);
	if (ret == FAIL) {
	...
	while (retry++ < 5) {
		show_len = 10;
		total_len = 100;
		ret = gup_burn_dsp_isp(gtp_sec_i2c_connect_client);
		if (ret == FAIL) {
		...
		show_len += 10;
		ret = gup_burn_fw_ss51(gtp_sec_i2c_connect_client);
		if (ret == FAIL) {
		...
		show_len += 40;
		ret = gup_burn_fw_dsp(gtp_sec_i2c_connect_client);
		if (ret == FAIL) {
		...
		show_len += 20;
		ret = gup_burn_fw_boot(gtp_sec_i2c_connect_client);
		if (ret == FAIL) {
	...
	gup_leave_update_mode(gtp_sec_i2c_connect_client);
